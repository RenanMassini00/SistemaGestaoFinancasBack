using Financeiro.Application.Common; using Financeiro.Application.DTOs;
namespace Financeiro.Application.Interfaces;
public interface IJwtTokenGenerator { AuthResponseDto Generate(Guid userId,string name,string email,string role); }
public interface IAuthService { Task<Result<AuthResponseDto>> LoginAsync(LoginRequestDto request,CancellationToken cancellationToken=default); }
...
namespace Financeiro.Application.Interfaces.Security; public interface IPasswordHasher { string Hash(string input); bool Verify(string input,string hash); }
